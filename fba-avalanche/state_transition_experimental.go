...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if contractCreation {
		...
		if (*msg.To() == stateConnectorContractAddr) {
			floorgval := new(big.Int).Mul(new(big.Int).SetUint64(flare.FixedGasFloor), st.gasPrice)
			if st.state.GetBalance(st.msg.From()).Cmp(floorgval) < 0 {
				...
				ret, _, vmerr = st.evm.Call(sender, st.to(), st.data, st.gas, st.value)
				st.gas = flare.FixedGas
			}
		} else {
	...
	st.state.AddBalance(feePoolContractAddr, new(big.Int).Mul(new(big.Int).SetUint64(flare.FixedGas), st.gasPrice))
