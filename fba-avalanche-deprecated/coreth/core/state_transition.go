...
func (st *StateTransition) buyGas() error {
	FixedGasMaxTotal := new(big.Int).Mul(new(big.Int).SetUint64(flare.FixedGasMax), st.gasPrice)
	...
	FixedGasTotal := new(big.Int).Mul(new(big.Int).SetUint64(flare.FixedGas), st.gasPrice)
