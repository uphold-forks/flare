...
nohup ./build/avalanchego --public-ip=127.0.0.1 --snow-sample-size=2 --snow-quorum-size=2 --http-port=9650 --staking-port=9651 --db-dir=db/node00/ --staking-enabled=true --network-id=ftsomvp --bootstrap-ips= --bootstrap-ids= --staking-tls-cert-file=$(pwd)/config/keys/node00/node.crt --staking-tls-key-file=$(pwd)/config/keys/node00/node.key --log-level=info --validators-file=$(pwd)/config/validators/validators_0000.json --alert-apis="https://flare.network" --xrp-apis=$XRP_APIs_JOINED &> $LOG_DIR/node00/launch.out & echo $! > $LOG_DIR/node00/launch.pid
NODE_00_PID=`cat $LOG_DIR/node00/launch.pid`
sleep 5

# NODE 2
printf "Launching Node 2 at 127.0.0.1:9652\n"
nohup ./build/avalanchego --public-ip=127.0.0.1 --snow-sample-size=2 --snow-quorum-size=2 --http-port=9652 --staking-port=9653 --db-dir=db/node01/ --staking-enabled=true --network-id=ftsomvp --bootstrap-ips=127.0.0.1:9651 --bootstrap-ids=$(cat $(pwd)/config/keys/node00/node.txt) --staking-tls-cert-file=$(pwd)/config/keys/node01/node.crt --staking-tls-key-file=$(pwd)/config/keys/node01/node.key --log-level=info --validators-file=$(pwd)/config/validators/validators_0000.json --alert-apis="https://flare.network" --xrp-apis=$XRP_APIs_JOINED --coreth-config="api-disabled" &> $LOG_DIR/node01/launch.out & echo $! > $LOG_DIR/node01/launch.pid
...
nohup ./build/avalanchego --public-ip=127.0.0.1 --snow-sample-size=2 --snow-quorum-size=2 --http-port=9656 --staking-port=9657 --db-dir=db/node03/ --staking-enabled=true --network-id=ftsomvp --bootstrap-ips=127.0.0.1:9651 --bootstrap-ids=$(cat $(pwd)/config/keys/node00/node.txt) --staking-tls-cert-file=$(pwd)/config/keys/node03/node.crt --staking-tls-key-file=$(pwd)/config/keys/node03/node.key --log-level=info --validators-file=$(pwd)/config/validators/validators_0000.json --alert-apis="https://flare.network" --xrp-apis=$XRP_APIs_JOINED --coreth-config="api-disabled" &> $LOG_DIR/node03/launch.out & echo $! > $LOG_DIR/node03/launch.pid
